...
public class AuthServiceTest {
    ...
    static void register_savesUser_returnsUser() {
        ...
        // act
        User saved = f.service.register(new RegisterCommand(name, rawEmail, rawPassword));
        ...
        assertThat(userToSave.name()).isEqualTo(name);
        assertThat(userToSave.profileImageUrl()).isNull();
    ...
    static void register_existingEmail_throwsException() {
        ...
        String normalizedEmail = "user11@test.com";
        User existing = new User(1L, "John Doe", null, normalizedEmail, "hash", Role.USER, Instant.now());

        when(f.userRepository.findByEmail(normalizedEmail)).thenReturn(Optional.of(existing));

        assertThatThrownBy(() -> f.service.register(new RegisterCommand("John Doe", "USER11@test.com", "pass")))
    ...
    static void login_success_returnsToken() {
        ...
        String normalizedEmail = "user11@test.com";
        User user = new User(10L, "John Doe", null, normalizedEmail, "HASHED", Role.USER, Instant.now());
